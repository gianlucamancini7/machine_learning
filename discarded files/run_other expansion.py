...
degrees=np.linspace(5,15,6).astype('int')
k_fold=4
mses= np.zeros((len(degrees), len(lambdas),2))
print('STARTING OPTIMIZATION')
for ind_degree,degree in enumerate(degrees):
    tx_train_selection_polynomial=poly_various_features_simple(df_train_selection, degree, other_order,log=False)
...
tx_train_selection_polynomial=poly_various_features_simple(df_train_selection, degree_final, other_order,log=False)
...
tx_test_selection_polynomial=poly_various_features_simple(df_test_selection, degree_final, other_order,log=False)

tx_test=tx_test_selection_polynomial
y_pred=predict_labels(w_final,tx_test)
create_csv_submission(df_test['Id'], y_pred, 'trial_sin_and_polynomial')
